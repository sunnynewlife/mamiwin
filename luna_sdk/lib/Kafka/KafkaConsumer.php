...
LunaLoader::import("luna_lib.Kafka.KafkaClient");
...
LunaLoader::import("luna_lib.Kafka.Protocol.Fetch.Helper.KafkaFreeStream");
LunaLoader::import("luna_lib.Kafka.Protocol.Fetch.Helper.KafkaHelper");
LunaLoader::import("luna_lib.Kafka.Protocol.Fetch.Helper.KafkaCommitOffset");
...
class KafkaConsumer
{
    ...
    {
        $zookeeper = new KafkaZooKeeper($hostList, $timeout);
        $this->client = new KafkaClient($zookeeper);
    }
    ...
    public function setPartition($topicName, $partitionId = 0, $offset = null)
    {
        if (is_null($offset)) {
            if ($this->fromOffset) {
                ...
                $offset = $offsetObject->getOffset(KafkaOffset::DEFAULT_EARLY);
                KafkaLog::log('topic name:' . $topicName . ', part:' . $partitionId . 'get offset from kafka server, offet:' . $offset, LOG_DEBUG);
            } else {
                $offset = 0;
            }
        }
    ...
    public function fetch()
    {
        ...
        $fetch = new KafkaTopic($streams, $data);

        // register fetch helper
        $freeStream = new KafkaFreeStream($this->client);
        $freeStream->setStreams($streams);
        KafkaHelper::registerHelper('freeStream', $freeStream);

        // register partition commit offset
        $commitOffset = new KafkaCommitOffset($this->client);
        $commitOffset->setGroup($this->group);
        KafkaHelper::registerHelper('commitOffset', $commitOffset);
