...
LunaLoader::import("luna_lib.Kafka.Exception.KafkaExceptionProtocol");
LunaLoader::import("luna_lib.Kafka.Exception.KafkaExceptionNotSupported");
LunaLoader::import("luna_lib.Kafka.Protocol.KafkaProtocol");

class KafkaEncoder extends KafkaProtocol
{
    // {{{ functions
    ...
    public function produceRequest($payloads, $compression = self::COMPRESSION_NONE)
    {
        if (!isset($payloads['data'])) {
            throw new KafkaExceptionProtocol('given procude data invalid. `data` is undefined.');
        }
    ...
    public function metadataRequest($topics)
    {
        ...
        foreach ($topics as $topic) {
            if (!is_string($topic)) {
                throw new KafkaExceptionProtocol('request metadata topic array have invalid value. ');
            }
        }
    ...
    {
        if (!isset($payloads['data'])) {
            throw new KafkaExceptionProtocol('given fetch kafka data invalid. `data` is undefined.');
        }
    ...
    {
        if (!isset($payloads['data'])) {
            throw new KafkaExceptionProtocol('given offset data invalid. `data` is undefined.');
        }
    ...
    {
        if (!isset($payloads['data'])) {
            throw new KafkaExceptionProtocol('given commit offset data invalid. `data` is undefined.');
        }

        if (!isset($payloads['group_id'])) {
            throw new KafkaExceptionProtocol('given commit offset data invalid. `group_id` is undefined.');
        }
    ...
    {
        if (!isset($payloads['data'])) {
            throw new KafkaExceptionProtocol('given fetch offset data invalid. `data` is undefined.');
        }

        if (!isset($payloads['group_id'])) {
            throw new KafkaExceptionProtocol('given fetch offset data invalid. `group_id` is undefined.');
        }
    ...
    public static function encodeString($string, $bytes, $compression = self::COMPRESSION_NONE)
    {
        ...
        switch ($compression) {
            ...
            case self::COMPRESSION_SNAPPY:
                throw new KafkaExceptionNotSupported('SNAPPY compression not yet implemented');
            default:
                throw new KafkaExceptionNotSupported('Unknown compression flag: ' . $compression);
        }
    ...
    public static function encodeArray(array $array, $func, $options = null)
    {
        if (!is_callable($func, false)) {
            throw new KafkaExceptionProtocol('Encode array failed, given function is not callable.');
        }
    ...
    protected static function _encodeProcudePartion($values, $compression)
    {
        if (!isset($values['partition_id'])) {
            throw new KafkaExceptionProtocol('given produce data invalid. `partition_id` is undefined.');
        }

        if (!isset($values['messages']) || empty($values['messages'])) {
            throw new KafkaExceptionProtocol('given produce data invalid. `messages` is undefined.');
        }
    ...
    protected static function _encodeProcudeTopic($values, $compression)
    {
        if (!isset($values['topic_name'])) {
            throw new KafkaExceptionProtocol('given produce data invalid. `topic_name` is undefined.');
        }

        if (!isset($values['partitions']) || empty($values['partitions'])) {
            throw new KafkaExceptionProtocol('given produce data invalid. `partitions` is undefined.');
        }
    ...
    {
        if (!isset($values['partition_id'])) {
            throw new KafkaExceptionProtocol('given fetch data invalid. `partition_id` is undefined.');
        }
    ...
    {
        if (!isset($values['topic_name'])) {
            throw new KafkaExceptionProtocol('given fetch data invalid. `topic_name` is undefined.');
        }

        if (!isset($values['partitions']) || empty($values['partitions'])) {
            throw new KafkaExceptionProtocol('given fetch data invalid. `partitions` is undefined.');
        }
    ...
    {
        if (!isset($values['partition_id'])) {
            throw new KafkaExceptionProtocol('given offset data invalid. `partition_id` is undefined.');
        }
    ...
    {
        if (!isset($values['topic_name'])) {
            throw new KafkaExceptionProtocol('given offset data invalid. `topic_name` is undefined.');
        }

        if (!isset($values['partitions']) || empty($values['partitions'])) {
            throw new KafkaExceptionProtocol('given offset data invalid. `partitions` is undefined.');
        }
    ...
    {
        if (!isset($values['partition_id'])) {
            throw new KafkaExceptionProtocol('given commit offset data invalid. `partition_id` is undefined.');
        }

        if (!isset($values['offset'])) {
            throw new KafkaExceptionProtocol('given commit offset data invalid. `offset` is undefined.');
        }
    ...
    {
        if (!isset($values['topic_name'])) {
            throw new KafkaExceptionProtocol('given commit offset data invalid. `topic_name` is undefined.');
        }

        if (!isset($values['partitions']) || empty($values['partitions'])) {
            throw new KafkaExceptionProtocol('given commit offset data invalid. `partitions` is undefined.');
        }
    ...
    {
        if (!isset($values['partition_id'])) {
            throw new KafkaExceptionProtocol('given fetch offset data invalid. `partition_id` is undefined.');
        }
    ...
    {
        if (!isset($values['topic_name'])) {
            throw new KafkaExceptionProtocol('given fetch offset data invalid. `topic_name` is undefined.');
        }

        if (!isset($values['partitions']) || empty($values['partitions'])) {
            throw new KafkaExceptionProtocol('given fetch offset data invalid. `partitions` is undefined.');
        }
