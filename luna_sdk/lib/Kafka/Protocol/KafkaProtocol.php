...
abstract class KafkaProtocol
{
     ...
     * @return void
     */
    public function __construct(KafkaSocket $stream)
    {
        $this->stream = $stream;
    }
    ...
    protected static function checkLen($type, $bytes)
    {
        ...
        if (strlen($bytes) != $len) {
            throw new KafkaExceptionProtocol('unpack failed. string(raw) length is ' . strlen($bytes) . ' , TO ' . $type);
        }
    }
