<div class="btn-toolbar">
	<a href="/gAppActivity/add?os_type=<?php echo Yii::app()->request->getParam('os_type',1);?>" class="btn btn-primary"><i class="icon-plus"></i>增加活动信息配置</a>
	<a data-toggle="collapse" data-target="#search"  href="#" title= "检索"><button class="btn btn-primary" style="margin-left:5px"><i class="icon-search"></i></button></a>	
</div>
<div id="search">
<form class="form_search"  action="/gAppActivity/index" method="GET" style="margin-bottom:0px">
...
<tr>
	...
	<td>%s</td>
	<td>
		<a href="/gAppActivity/modify?activity_id=%s&gameid=%s" title="修改"><i class="icon-pencil"></i></a>&nbsp;
		<a data-toggle="modal" href="#myModal" title="删除"><i class="icon-remove" href="/gAppActivity/del?activity_id=%s&gameid=%s"></i></a>
	</td>
<tr>
EndOfRowTag;
				$os_type=Yii::app()->request->getParam('os_type',1);
			 	...
			 	foreach ($t_game_activity as $item){
			 		if(isset($game_name_list[$item["gameid"]])){
						$rowHtml=sprintf($rowsHtmlTag,
							...
							$state[$item["state"]],
							$item["activity_id"],$item["gameid"],$item["activity_id"],$item["gameid"]);
						echo $rowHtml; 
			 		}else{
