...
class AlipaySubmit {
	...
	function buildRequestPara($para_temp) {
		...
		//2015-5-25 加入_input_charset进行签名
		$para_filter = array_merge($para_filter,
				array("_input_charset" => $this->alipay_config['input_charset'])
			);
		//对待签名参数数组排序
	...
	function buildRequestHttp($para_temp) {
		...
		logResult("alipayTransRequest=".json_encode($request_data));
		
		//远程获取数据
		$url = $this->alipay_gateway_new . http_build_query($request_data);
		LunaLogger::getInstance()->info("alipay_url:".$url);	
		...
		var_dump($sResult);
		logResult("alipayTransResult=".($sResult));		
		return $sResult;
	}
